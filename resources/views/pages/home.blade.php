...
@section('content')
    <section class="min-h-screen bg-slate-900/75 flex flex-col justify-center relative px-10">
        <img src="{{ $header->imageUrl }}" alt=""
            class="absolute top-0 left-0 w-full h-full object-cover opacity-30 -z-10">
        <div class="container mx-auto">
            <span class="text-blue-400 text-lg uppercase leading-10">{{ $header->sub_title }}</span>
            <h1 class="text-5xl font-semibold text-white leading-14 w-full md:max-w-4xl">{{ $header->title }}</h1>
            <p class="text-gray-300 w-full md:max-w-3xl py-5">{{ $header->description }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
            @foreach ($services_list as $item)
                <div class="bg-transparent ">
                    ...
                    <p class="text-gray-600 text-left">{{ \Illuminate\Support\Str::words(html_entity_decode(strip_tags($item->description)), 20) }}</p>
                    <a href="{{ route('service', $item->slug) }}" class="text-blue-500 mt-2 flex  items-center gap-1 hover:gap-3 transition-all">Learn more <ion-icon name="arrow-forward-outline" class="mt-1 text-md"></ion-icon></a>
                </div>
                {{-- <div class="bg-slate-100 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow py-10">
                    <img src="./img/services.png" alt="" class="w-32 h-32 mb-4 mx-auto">
                    <h3 class="text-xl font-semibold mb-4 text-slate-700 text-center">Tax Consultation & Preparation
                    </h3>
                    <p class="text-gray-600 text-center">Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                        Quisque nisl eros, pulvinar facilisis justo mollis.</p>
                </div> --}}
            @endforeach
        </div>
    </section>
    ...
    <section class="">
        <div class="flex flex-row items-center justify-center gap-10 px-10 py-20 bg-gray-100">
            <div class="relative z-20">
                <img class="w-[500px] h-[600px] object-cover rounded-4xl z-20" src="{{ $philosophy->imageUrl }}"
                    alt="">
                <div
            ...
            <div class="">
                <h2 class="text-5xl font-bold mb-4 text-slate-600">{{ $philosophy->title }}</h2>
    ...
    <section class="">
        <div class="flex flex-row items-center justify-center gap-10 px-10 py-20">
            <div class="">
                <h2 class="text-5xl font-bold mb-4 text-slate-600">{{ $appointment->title }}</h2>
                <p class="text-lg text-gray-500 mb-6 max-w-2xl">
    ...
    <section>
        <div class="w-full h-screen flex items-center justify-center rounded-lg p-6 z-20 bg-gray-100">
