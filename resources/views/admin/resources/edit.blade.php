...
@section('content')
    ...
    <div class="w-full py-22">
        ...
        <form action="{{ route('admin.resources.update', $resource->id) }}" class="border border-slate-700 w-2/3 mx-auto p-4 px-6 rounded-lg" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <label for="title" class="block font-light my-2 text-slate-100">
                ...
                <input type="title" id="title"
                    ...
                    placeholder="Enter title here" value="{{ old('title') ?? $resource->title }}" name="title" />
            ...
            <label for="url" class="block font-light my-2 text-slate-100">
                ...
                <input type="url" id="url"
                    ...
                    placeholder="Enter URL here" value="{{ old('url') ?? $resource->url }}" name="url" />
            ...
            <label for="tags" class="block font-light my-2 text-slate-100">
                ...
                <input type="tags" id="tags"
                    ...
                    placeholder="Enter tags separated by commas" value="{{ old('tags') ?? $resource->tags }}" name="tags" />
            ...
            <label for="description" class="block font-light my-2 text-slate-100">
                ...
                <textarea name="description" id="description" cols="30" rows="5" class="mt-0.5 w-full rounded shadow-sm sm:text-sm border @error('description') border-red-300 @else border-gray-600 @enderror bg-gray-900 text-white py-2 px-3" placeholder="Enter description here">{{ old('description') ?? $resource->description }}</textarea>
